    ...
    async (page = 1, { rejectWithValue }) => {
        try {
            const res = await fetchWithAuth(
                `${import.meta.env.VITE_APP_API_URL}/api/todos?page=${page}&pageSize=6`
            );
            const data = await res.json();
...
export const createTodo = createAsyncThunk(
    ...
    async (payload, { rejectWithValue }) => {
        try {
            ...
            const res = await fetchWithAuth(`${import.meta.env.VITE_APP_API_URL}/api/todos`, {
                method: "POST",
                body: JSON.stringify({
                    ...values,
                    checked: false,
...
export const toggleTodo = createAsyncThunk(
    ...
    async (todo, { rejectWithValue }) => {
        try {
            const res = await fetchWithAuth(`${import.meta.env.VITE_APP_API_URL}/api/todos/${todo._id}`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                },
...
export const searchTodos = createAsyncThunk(
    ...
    async (query, { rejectWithValue }) => {
        try {
            const res = await fetchWithAuth(
                `${import.meta.env.VITE_APP_API_URL}/api/todos/search?query=${encodeURIComponent(query)}`
            );
